...
def test_export_adjusted_series_requires_materialized_factor(repo, tmp_path):
    InstrumentCollector(repo).collect(
    ...
    rows = json.loads((tmp_path / "out_adj_missing" / "instrument_daily.parquet").read_text(encoding="utf-8"))
    first_day = [r for r in rows if r["trade_date"] == "2026-01-02"][0]
    assert first_day["close"] == 100
